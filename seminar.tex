...
\begin{frame}
   ...
   \begin{enumerate}
       \item  $s$ sends a \emph{proposal}, containing a log entry and the next leader to the current side leader $l$. 
       If another transaction is executing, $t$ is aborted otherwise $l$ sends the proposal to other sites.
       \item $s$ waits for an \emph{ack} response from all sites. If some sites fail to responde, an \emph{invalidate} message is sent instead.
       \item When all sites acknowledge the proposal or the invalidate message, $s$ requests all sites to apply $t$ changes. 
